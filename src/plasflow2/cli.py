...
@click.option(
    ...
    type=click.Path(),
    help=(
        "Path to COMPASS MinHash sketch (.npy). When provided, enables the containment "
        "post-filter: plasmid predictions with no COMPASS k-mer overlap are reclassified "
        "as chromosome. Build sketch with: python scripts/build_comparative_sketches.py"
    ),
)
@click.option(
    "--compass-threshold",
    ...
    show_default=True,
    help=(
        "Minimum containment score (fraction of query k-mers in COMPASS sketch) to retain "
        "a plasmid prediction. Default 0.002 is the Tier 1-benchmarked optimum "
        "(plasmid F1=0.727 vs 0.534 baseline, 299,589-sequence benchmark, Jul 2026)."
    ),
)
@click.pass_context
def classify(
